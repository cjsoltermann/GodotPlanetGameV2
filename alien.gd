...
func _process(delta):
	if not target and get_closest_target:
...
func _physics_process(delta):
	if anchor_planet:
		if (anchor_planet.global_position - global_position).length_squared() < 2000:
			...
			var new_basis := Basis(new_x, up_direction, new_z).orthonormalized()
